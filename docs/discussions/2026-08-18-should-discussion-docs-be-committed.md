...
to `git fsck`. What the file was recovered from was an out-of-band snapshot (**6,358 B**); the
live file today is **24,556 B**. Those two numbers do *not* establish how much was lost — the
difference is equally consistent with content added after the restore. What they do establish is
that recovery depended on something outside git entirely.
...
- *Machine-specific paths* — grepped `/home/`, `/Users/`, `/nix/store` across all four tracked
  docs **and** the untracked scratch file: **0 hits in every file.** Hypothetical *in today's
  corpus*. It is not a property of future records — see constraint 4.
...
**No conflict. No error. No warning. Zero *git* recovery paths found, and A's 15 KB is gone from
the working tree.** Scope of that claim: the three git mechanisms tested — `git checkout --`,
reflog, `fsck` dangling blobs — all came back empty. Filesystem snapshots, editor undo history,
external backups and undelete tooling were **not** tested and may or may not have held a copy;
that is exactly the out-of-band luck the real incident ran on. The argument is about git: tracking
does not prevent the collision, it converts a loss git cannot see into one git makes visible and
resolvable.
...
4. **Every record gets a disclosure review before it is committed.** Read the file for secrets,
   credentials and tokens; personal data; private infrastructure detail (internal hostnames, IPs,
   room IDs, unpublished endpoints); and machine-specific paths. A commit to a repo with any
   public-facing remote is effectively permanent and unredactable — rewriting history does not
   recall a fetched object. The measured "0 hits across the current corpus" in §4 is a fact about
   five files that already exist; it says nothing about what a future generated record will
   contain, and these records are generated by agents that read the whole machine. This is a
   per-record gate, not a one-time finding.
5. **Never put "ignore" in a filename in this repo.** `.gitignore:15` is `*ignore*`, unanchored —
