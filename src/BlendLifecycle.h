...
inline QVariantMap reduce(const QVariantMap& in)
{
    ...
    if (!in.value("running").toBool())
        ...
        set("activation-pending", "Activation pending", "Activation starts at created + 2 epochs, subject to the frozen membership snapshot.");
        ...
        set("activation-pending", "Waiting for Core membership", "The runtime is not Core. Frozen membership or the network's minimum member threshold may still prevent activation. Stay online; do not redeclare.");
